...
umi_histograms_folder="/data/public/jkoubele/sc_data/umi_histograms"
...
for sub_folder in "$aligned_folder"/"$gse"/*; do
  ...
  output_folder="$umi_histograms_folder"/"$gse"/"$sample_name"
  mkdir "$output_folder" -p
  sbatch --output="$slurm_log_folder"/%j_%x.log --error="$slurm_log_folder"/%j_%x.err \
  "$repository_path"/job_scripts/compute_umi_histograms.sh \
